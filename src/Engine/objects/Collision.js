...
export default class CollisionManager {
    ...
    registerCollision(colObjName){
        ...
        collisionHolder.installCollision(this._parent.name, colObjName, {text: '', code: ''});
    }

    setCollision(colName, code){
        if(!this._regCollisions[colName]) throw Error('setting a collision that is\'t registered');
    ...
    removeCollision(colName) {
        if(!this._regCollisions[colName])throw Error('trying to remove a collision that doesn\'t exist');
